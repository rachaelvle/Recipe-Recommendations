...
import { DIETS, ALLERGIES, CUISINES } from '../constants/FilterOptions';
...
export default function HomeScreen({ navigation }) { 
  ...
  useEffect(() => {
    ...
    if (activeFilters.maxTime) {
      let minutes = activeFilters.maxTime;
      
      // Safety Check: Only use .replace if it is actually a string
...
// --- HEADER COMPONENT ---
  const ListHeader = useMemo(() => {
    ...
    const isFiltering = 
      ...
      activeFilters.maxTime !== '60 min' || // Ignore if it's the default 60
      activeFilters.diets.length > 0 ||     // Check if diets are selected
      activeFilters.allergies.length > 0;   // Check if allergies are selected
...
<View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 10 }}>
  {DIETS.map((diet) => {
    const isActive = activeFilters.diets.includes(diet);
    return (
      <TouchableOpacity
        key={diet}
        onPress={() => toggleFilter('diets', diet)}
        style={{
          backgroundColor: isActive ? '#4ade80' : '#f3f4f6', // Green if active
          paddingVertical: 8,
      ...
      >
        <Text style={{ color: isActive ? 'white' : 'black', fontWeight: '600' }}>
          {diet}
        </Text>
      </TouchableOpacity>
    );
...
<View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 10 }}>
  {ALLERGIES.map((allergy) => {
    const isActive = activeFilters.allergies.includes(allergy);
    return (
      <TouchableOpacity
        key={allergy}
        onPress={() => toggleFilter('allergies', allergy)}
        style={{
          backgroundColor: isActive ? '#ef4444' : '#f3f4f6', // Red if active
          paddingVertical: 8,
          paddingHorizontal: 16,
          borderRadius: 20,
          borderWidth: 1,
      ...
      >
        <Text style={{ color: isActive ? 'white' : 'black', fontWeight: '600' }}>
          {allergy}
                  ...
                  onPress={() => {
                    // Reset values but KEEP the array structure for diets/allergies
                    setActiveFilters({ 
                      difficulty: null, 
                      maxTime: '60 min', 
                      cuisine: null,
